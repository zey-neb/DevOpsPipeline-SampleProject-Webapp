           ...
           stage('Build Docker image')
            {
                  ...
                  stage ('Start and run container')
                {
                    steps
                    {
                        sh'docker stop webapp_${BUILD_NUMBER}-1; docker stop webapp_${BUILD_NUMBER}-1 ; docker run -d -p ${BUILD_NUMBER}:8080 --name webapp_${BUILD_NUMBER} webapp:${BUILD_NUMBER}'
                    }
                }
                stage('Katalon')
                {
